...
function get_name_value_list($value, $returnDomValue = false){
	...
	if(!empty($value->field_defs)){
		...
		foreach($value->field_defs as $var){
			if( /* BEGIN ModuleSurfer ::TAG 0121r75j:: */
!(isset($GLOBALS['TSWrapper']) && ($var['name'] == 'name' || $var['type'] == 'id' || $var['type'] == 'parent' || $var['type'] == 'link')) &&
/* END ModuleSurfer ::TAG 0121r75j:: */ isset($var['source']) && ($var['source'] != 'db' && $var['source'] != 'custom_fields') && $var['name'] != 'email1' && $var['name'] != 'email2' && (!isset($var['type'])|| $var['type'] != 'relate')){
