	...
	function process($file, $data, $htmlVar) {
        ...
        if($this->show_action_dropdown)
		{
            ...
            if(count($action_menu['buttons']) > 0) {
                 /* BEGIN ModuleSurfer ::TAG 0121r7c2:: */
                foreach($action_menu['buttons'] as $i => $v) {
                    if ($v) break;
                }
                $this->ss->assign('actionDisabledLink', preg_replace("/id\s*\=(\"\w+\"|w+)/i", "", $action_menu['buttons'][$i]));
/* END ModuleSurfer ::TAG 0121r7c2:: */
}
