...
$experian = $row['experian_deleted'];
$equifax=$row['equifax_deleted'];
$transunion = $row['transunion_deleted'];
$total = $experian+$equifax+$transunion;
$second_number = 3;
$sum_total = $num_rows * $second_number;
$finalval = $sum_total-$total;

echo"<h4><b>No Of creditors:</b>$num_rows</h4><h4><b>Total:</b>$sum_total</h4><h4><b>Final Value:</b>$finalval</h4>";
    ...
    function uniqueids(){
      if (function_exists('com_create_guid')){
       return com_create_guid();
         ...
         $sql="SELECT * FROM  `cases` WHERE  `account_id` ='".$result['account_id']."' ORDER BY  `date_entered` DESC";
