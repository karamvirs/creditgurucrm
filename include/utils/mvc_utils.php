...
function ajaxLink($url)
{
    ...
    preg_match('/^javascript/i', $url, $javascriptMatch);
     ...
     /* BEGIN ModuleSurfer ::TAG 0121r7bt:: */
    if ($match[1] != 'Home' && file_exists('custom/include/ModuleSurfer/ModuleSurferManager.php')) {
        require_once('custom/include/ModuleSurfer/ModuleSurferManager.php');
        $moduleSurferSettings = ModuleSurferManager::loadSettings($match[1]);
        if (!$moduleSurferSettings['disabled'] && (!isset($sugar_config['dispageModuleSurfer']) || !isset($sugar_config['dispageModuleSurfer'][$match[1]]) || empty($sugar_config['dispageModuleSurfer'][$match[1]]['disable']))) {
            return $url;
        }
    }
/* END ModuleSurfer ::TAG 0121r7bt:: */
