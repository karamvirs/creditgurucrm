...
return c};this._compareValues=function(n,l,p,A,t){var H;H=M?"this."+l:"this";if(p===undefined)p=null;p=p===null?l:p;switch(t.stype===undefined?"text":t.stype){case "int":case "integer":p=isNaN(Number(p))?"0":p;H="parseInt("+H+",10)";p="parseInt("+p+",10)";break;case "float":case "number":case "numeric":p=String(p).replace(r,"");p=isNaN(Number(p))?"0":p;H="parseFloat("+H+")";p="parseFloat("+p+")";break;case "date":case "datetime":p=String(b.jgrid.parseDate(t.newfmt||"Y-m-d",p).getTime());H='djQuery.jgrid.parseDate("'+
...
[];j.p_index={};j.updatepager(true,false)}})},getInd:function(f,j){var i=false,c;this.each(function(){if(c=this.rows.namedItem(f))i=j===true?c:c.rowIndex});return i}})})(djQuery);
...
d.split(";"),j=0,k=0;k<i.length;k++){f=i[k].split(":");if(f.length>2)f[1]=djQuery.map(f,function(l,o){if(o>0)return l}).join(":");if(h){if(djQuery.inArray(f[0],g)>-1){e[j]=f[1];j++}}else if(c.trim(f[0])==c.trim(a)){e[0]=f[1];break}}else if(c.fmatter.isObject(d))if(h)e=djQuery.map(g,function(l){return d[l]});else e[0]=d[a]||""}a=e.join(", ");return a===""?c.fn.fmatter.defaultFormat(a,b):a};c.fn.fmatter.rowactions=function(a,b,d,e){switch(d){case "edit":d=function(){c("tr#"+a+" div.ui-inline-edit, tr#"+
...
"onclick=d$.fn.fmatter.rowactions('"+d+"','"+b.gid+"','edit',"+a.keys+");";e=e+"<div style='margin-left:8px;'><div title='"+c.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+h+"><span class='ui-icon ui-icon-pencil'></span></div>"}if(a.delbutton){h="onclick=djQuery('#"+b.gid+"').jqGrid('delGridRow','"+d+"');";e=e+"<div title='"+c.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+h+"><span class='ui-icon ui-icon-trash'></span></div>"}h=
"onclick=d$.fn.fmatter.rowactions('"+d+"','"+b.gid+"','save',false);";e=e+"<div title='"+c.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save'><span class='ui-icon ui-icon-disk' "+h+"></span></div>";h="onclick=d$.fn.fmatter.rowactions('"+d+"','"+b.gid+"','cancel',false);";return e=e+"<div title='"+c.jgrid.edit.bCancel+"' style='float:left;display:none;margin-left:5px;' class='ui-pg-div ui-inline-cancel'><span class='ui-icon ui-icon-cancel' "+h+"></span></div></div>"};
...
c.unformat.select(a,b,d,e);break;case "actions":return"";default:h=c(a).text()}}return h?h:e===true?c(a).text():c.jgrid.htmlDecode(c(a).html())};c.unformat.select=function(a,b,d,e){d=[];a=c(a).text();if(e===true)return a;b=c.extend({},b.colModel.editoptions);if(b.value){var h=b.value;b=b.multiple===true?true:false;e=[];var g;if(b){e=a.split(",");e=c.map(e,function(k){return c.trim(k)})}if(c.fmatter.isString(h))for(var f=h.split(";"),i=0,j=0;j<f.length;j++){g=f[j].split(":");if(g.length>2)g[1]=djQuery.map(g,
function(k,l){if(l>0)return k}).join(":");if(b){if(djQuery.inArray(g[1],e)>-1){d[i]=g[0];i++}}else if(c.trim(g[1])==c.trim(a)){d[0]=g[0];break}}else if(c.fmatter.isObject(h)||c.isArray(h)){b||(e[0]=a);d=djQuery.map(e,function(k){var l;c.each(h,function(o,n){if(n==k){l=o;return false}});if(typeof l!="undefined")return l})}return d.join(", ")}else return a||""};c.unformat.date=function(a,b){var d=c.jgrid.formatter.date||{};c.fmatter.isUndefined(b.formatoptions)||(d=c.extend({},d,b.formatoptions));return c.fmatter.isEmpty(a)?
c.fn.fmatter.defaultFormat(a,b):c.fmatter.util.DateFormat(d.newformat,a,d.srcformat,d)}})(djQuery);
...
this.triggerSearch=h;this.clearSearch=k}else alert("Could not get grid colModel")}else alert("No target grid is set!")})},filterToolbar:function(f){f=a.extend({autosearch:true,searchOnEnter:true,beforeSearch:null,afterSearch:null,beforeClear:null,afterClear:null,searchurl:"",stringResult:false,groupOp:"AND",defaultSearch:"bw"},f||{});return this.each(function(){function d(h,k){var o=a(h);o[0]&&djQuery.each(k,function(){this.data!==undefined?o.bind(this.type,this.data,this.fn):o.bind(this.type,this.fn)})}
...
h.css("display")=="none"?h.show():h.hide()}})}})})(djQuery);
var showModal=function(a){a.w.show()},closeModal=function(a){a.w.hide().attr("aria-hidden","true");a.o&&a.o.remove()},hideModal=function(a,b){b=djQuery.extend({jqm:true,gb:""},b||{});if(b.onClose){var c=b.onClose(a);if(typeof c=="boolean"&&!c)return}if(djQuery.fn.jqm&&b.jqm===true)djQuery(a).attr("aria-hidden","true").jqmHide();else{if(b.gb!=="")try{djQuery(".jqgrid-overlay:first",b.gb).hide()}catch(e){}djQuery(a).hide().attr("aria-hidden","true")}};
function findPos(a){var b=0,c=0;if(a.offsetParent){do{b+=a.offsetLeft;c+=a.offsetTop}while(a=a.offsetParent)}return[b,c]}
var createModal=function(a,b,c,e,f,g){var d=document.createElement("div"),h;h=djQuery(c.gbox).attr("dir")=="rtl"?true:false;d.className="ui-widget ui-widget-content ui-corner-all ui-jqdialog";d.id=a.themodal;var i=document.createElement("div");i.className="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix";i.id=a.modalhead;djQuery(i).append("<span class='ui-jqdialog-title'>"+c.caption+"</span>");var j=djQuery("<a href='javascript:void(0)' class='ui-jqdialog-titlebar-close ui-corner-all'></a>").hover(function(){j.addClass("ui-state-hover")},
function(){j.removeClass("ui-state-hover")}).append("<span class='ui-icon ui-icon-closethick'></span>");djQuery(i).append(j);if(h){d.dir="rtl";djQuery(".ui-jqdialog-title",i).css("float","right");djQuery(".ui-jqdialog-titlebar-close",i).css("left","0.3em")}else{d.dir="ltr";djQuery(".ui-jqdialog-title",i).css("float","left");djQuery(".ui-jqdialog-titlebar-close",i).css("right","0.3em")}var l=document.createElement("div");djQuery(l).addClass("ui-jqdialog-content ui-widget-content").attr("id",a.modalcontent);
djQuery(l).append(b);d.appendChild(l);djQuery(d).prepend(i);g===true?djQuery("body").append(d):djQuery(d).insertBefore(e);if(typeof c.jqModal==="undefined")c.jqModal=true;b={};if(djQuery.fn.jqm&&c.jqModal===true){if(c.left===0&&c.top===0){e=[];e=findPos(f);c.left=e[0]+4;c.top=e[1]+4}b.top=c.top+"px";b.left=c.left}else if(c.left!==0||c.top!==0){b.left=c.left;b.top=c.top+"px"}djQuery("a.ui-jqdialog-titlebar-close",i).click(function(){var n=djQuery("#"+a.themodal).data("onClose")||c.onClose,k=djQuery("#"+a.themodal).data("gbox")||
c.gbox;hideModal("#"+a.themodal,{gb:k,jqm:c.jqModal,onClose:n});return false});if(c.width===0||!c.width)c.width=300;if(c.height===0||!c.height)c.height=200;if(!c.zIndex)c.zIndex=950;f=0;if(h&&b.left&&!g){f=djQuery(c.gbox).width()-(!isNaN(c.width)?parseInt(c.width,10):0)-8;b.left=parseInt(b.left,10)+parseInt(f,10)}if(b.left)b.left+="px";djQuery(d).css(djQuery.extend({width:isNaN(c.width)?"auto":c.width+"px",height:isNaN(c.height)?"auto":c.height+"px",zIndex:c.zIndex,overflow:"hidden"},b)).attr({tabIndex:"-1",
role:"dialog","aria-labelledby":a.modalhead,"aria-hidden":"true"});if(typeof c.drag=="undefined")c.drag=true;if(typeof c.resize=="undefined")c.resize=true;if(c.drag){djQuery(i).css("cursor","move");if(djQuery.fn.jqDrag)djQuery(d).jqDrag(i);else try{djQuery(d).draggable({handle:djQuery("#"+i.id)})}catch(q){}}if(c.resize)if(djQuery.fn.jqResize){djQuery(d).append("<div class='jqResize ui-resizable-handle ui-resizable-se ui-icon ui-icon-gripsmall-diagonal-se ui-icon-grip-diagonal-se'></div>");djQuery("#"+a.themodal).jqResize(".jqResize",
a.scrollelm?"#"+a.scrollelm:false)}else try{djQuery(d).resizable({handles:"se, sw",alsoResize:a.scrollelm?"#"+a.scrollelm:false})}catch(o){}c.closeOnEscape===true&&djQuery(d).keydown(function(n){if(n.which==27){n=djQuery("#"+a.themodal).data("onClose")||c.onClose;hideModal(this,{gb:c.gbox,jqm:c.jqModal,onClose:n})}})},viewModal=function(a,b){b=djQuery.extend({toTop:true,overlay:10,modal:false,onShow:showModal,onHide:closeModal,gbox:"",jqm:true,jqM:true},b||{});if(djQuery.fn.jqm&&b.jqm===true)b.jqM?djQuery(a).attr("aria-hidden",
"false").jqm(b).jqmShow():djQuery(a).attr("aria-hidden","false").jqmShow();else{if(b.gbox!==""){djQuery(".jqgrid-overlay:first",b.gbox).show();djQuery(a).data("gbox",b.gbox)}djQuery(a).show().attr("aria-hidden","false");try{djQuery(":input:visible",a)[0].focus()}catch(c){}}};
function info_dialog(a,b,c,e){var f={width:290,height:"auto",dataheight:"auto",drag:true,resize:false,caption:"<b>"+a+"</b>",left:250,top:170,zIndex:1E3,jqModal:true,modal:false,closeOnEscape:true,align:"center",buttonalign:"center",buttons:[]};djQuery.extend(f,e||{});var g=f.jqModal;if(djQuery.fn.jqm&&!g)g=false;a="";if(f.buttons.length>0)for(e=0;e<f.buttons.length;e++){if(typeof f.buttons[e].id=="undefined")f.buttons[e].id="info_button_"+e;a+="<a href='javascript:void(0)' id='"+f.buttons[e].id+"' class='fm-button ui-state-default ui-corner-all'>"+
...
c+"</a>"+a+"</div>":a!==""?"<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+f.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>"+a+"</div>":"";d+="</div>";try{djQuery("#info_dialog").attr("aria-hidden")=="false"&&hideModal("#info_dialog",{jqm:g});djQuery("#info_dialog").remove()}catch(h){}createModal({themodal:"info_dialog",modalhead:"info_head",modalcontent:"info_content",scrollelm:"infocnt"},d,f,"","",true);a&&djQuery.each(f.buttons,
function(j){djQuery("#"+this.id,"#info_id").bind("click",function(){f.buttons[j].onClick.call(djQuery("#info_dialog"));return false})});djQuery("#closedialog","#info_id").click(function(j){hideModal("#info_dialog",{jqm:g});return false});djQuery(".fm-button","#info_dialog").hover(function(){djQuery(this).addClass("ui-state-hover")},function(){djQuery(this).removeClass("ui-state-hover")});djQuery.isFunction(f.beforeOpen)&&f.beforeOpen();viewModal("#info_dialog",{onHide:function(j){j.w.hide().remove();j.o&&
j.o.remove()},modal:f.modal,jqm:g});djQuery.isFunction(f.afterOpen)&&f.afterOpen();try{djQuery("#info_dialog").focus()}catch(i){}}
function createEl(a,b,c,e,f){function g(k,m){if(djQuery.isFunction(m.dataInit)){k.id=m.id;m.dataInit(k);delete m.id;delete m.dataInit}if(m.dataEvents){djQuery.each(m.dataEvents,function(){this.data!==undefined?djQuery(k).bind(this.type,this.data,this.fn):djQuery(k).bind(this.type,this.fn)});delete m.dataEvents}return m}var d="";b.defaultValue&&delete b.defaultValue;switch(a){case "textarea":d=document.createElement("textarea");if(e)b.cols||djQuery(d).css({width:"98%"});else if(!b.cols)b.cols=20;if(!b.rows)b.rows=
2;if(c=="&nbsp;"||c=="&#160;"||c.length==1&&c.charCodeAt(0)==160)c="";d.value=c;b=g(d,b);djQuery(d).attr(b).attr({role:"textbox",multiline:"true"});break;case "checkbox":d=document.createElement("input");d.type="checkbox";if(b.value){var h=b.value.split(":");if(c===h[0]){d.checked=true;d.defaultChecked=true}d.value=h[0];djQuery(d).attr("offval",h[1]);try{delete b.value}catch(i){}}else{h=c.toLowerCase();if(h.search(/(false|0|no|off|undefined)/i)<0&&h!==""){d.checked=true;d.defaultChecked=true;d.value=
c}else d.value="on";djQuery(d).attr("offval","off")}b=g(d,b);djQuery(d).attr(b).attr("role","checkbox");break;case "select":d=document.createElement("select");d.setAttribute("role","select");var j,l=[];if(b.multiple===true){j=true;d.multiple="multiple";djQuery(d).attr("aria-multiselectable","true")}else j=false;if(typeof b.dataUrl!="undefined")djQuery.ajax(djQuery.extend({url:b.dataUrl,type:"GET",dataType:"html",success:function(k,m){try{delete b.dataUrl;delete b.value}catch(r){}if(typeof b.buildSelect!=
"undefined"){k=b.buildSelect(k);k=djQuery(k).html();delete b.buildSelect}else k=djQuery(k).html();if(k){djQuery(d).append(k);b=g(d,b);if(typeof b.size==="undefined")b.size=j?3:1;if(j){l=c.split(",");l=djQuery.map(l,function(p){return djQuery.trim(p)})}else l[0]=djQuery.trim(c);djQuery(d).attr(b);setTimeout(function(){djQuery("option",d).each(function(p){if(p===0)this.selected="";djQuery(this).attr("role","option");if(djQuery.inArray(djQuery.trim(djQuery(this).text()),l)>-1||djQuery.inArray(djQuery.trim(djQuery(this).val()),
l)>-1){this.selected="selected";if(!j)return false}})},0)}}},f||{}));else if(b.value){if(j){l=c.split(",");l=djQuery.map(l,function(k){return djQuery.trim(k)});if(typeof b.size==="undefined")b.size=3}else b.size=1;if(typeof b.value==="function")b.value=b.value();if(typeof b.value==="string"){e=b.value.split(";");for(h=0;h<e.length;h++){f=e[h].split(":");if(f.length>2)f[1]=djQuery.map(f,function(k,m){if(m>0)return k}).join(":");a=document.createElement("option");a.setAttribute("role","option");a.value=
f[0];a.innerHTML=f[1];if(!j&&(djQuery.trim(f[0])==djQuery.trim(c)||djQuery.trim(f[1])==djQuery.trim(c)))a.selected="selected";if(j&&(djQuery.inArray(djQuery.trim(f[1]),l)>-1||djQuery.inArray(djQuery.trim(f[0]),l)>-1))a.selected="selected";d.appendChild(a)}}else if(typeof b.value==="object"){e=b.value;for(h in e)if(e.hasOwnProperty(h)){a=document.createElement("option");a.setAttribute("role","option");a.value=h;a.innerHTML=e[h];if(!j&&(djQuery.trim(h)==djQuery.trim(c)||djQuery.trim(e[h])==djQuery.trim(c)))a.selected=
"selected";if(j&&(djQuery.inArray(djQuery.trim(e[h]),l)>-1||djQuery.inArray(djQuery.trim(h),l)>-1))a.selected="selected";d.appendChild(a)}}b=g(d,b);try{delete b.value}catch(q){}djQuery(d).attr(b)}break;case "text":case "password":case "button":h=a=="button"?"button":"textbox";d=document.createElement("input");d.type=a;d.value=c;b=g(d,b);if(a!="button")if(e)b.size||djQuery(d).css({width:"98%"});else if(!b.size)b.size=20;djQuery(d).attr(b).attr("role",h);break;case "image":case "file":d=document.createElement("input");
d.type=a;b=g(d,b);djQuery(d).attr(b);break;case "custom":d=document.createElement("span");try{if(djQuery.isFunction(b.custom_element)){var o=b.custom_element.call(this,c,b);if(o){o=djQuery(o).addClass("customelement").attr({id:b.id,name:b.name});djQuery(d).empty().append(o)}else throw"e2";}else throw"e1";}catch(n){n=="e1"&&info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_element' "+djQuery.jgrid.edit.msg.nodefined,djQuery.jgrid.edit.bClose);n=="e2"?info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_element' "+
djQuery.jgrid.edit.msg.novalue,djQuery.jgrid.edit.bClose):info_dialog(djQuery.jgrid.errors.errcap,typeof n==="string"?n:n.message,djQuery.jgrid.edit.bClose)}break}return d}function daysInFebruary(a){return a%4===0&&(a%100!==0||a%400===0)?29:28}function DaysArray(a){for(var b=1;b<=a;b++){this[b]=31;if(b==4||b==6||b==9||b==11)this[b]=30;if(b==2)this[b]=29}return this}
...
function checkValues(a,b,c){var e,f,g,d;if(typeof b=="string"){f=0;for(d=c.p.colModel.length;f<d;f++)if(c.p.colModel[f].name==b){e=c.p.colModel[f].editrules;b=f;try{g=c.p.colModel[f].formoptions.label}catch(h){}break}}else if(b>=0)e=c.p.colModel[b].editrules;if(e){g||(g=c.p.colNames[b]);if(e.required===true)if(isEmpty(a))return[false,g+": "+djQuery.jgrid.edit.msg.required,""];f=e.required===false?false:true;if(e.number===true)if(!(f===false&&isEmpty(a)))if(isNaN(a))return[false,g+": "+djQuery.jgrid.edit.msg.number,
""];if(typeof e.minValue!="undefined"&&!isNaN(e.minValue))if(parseFloat(a)<parseFloat(e.minValue))return[false,g+": "+djQuery.jgrid.edit.msg.minValue+" "+e.minValue,""];if(typeof e.maxValue!="undefined"&&!isNaN(e.maxValue))if(parseFloat(a)>parseFloat(e.maxValue))return[false,g+": "+djQuery.jgrid.edit.msg.maxValue+" "+e.maxValue,""];if(e.email===true)if(!(f===false&&isEmpty(a))){d=/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i;
if(!d.test(a))return[false,g+": "+djQuery.jgrid.edit.msg.email,""]}if(e.integer===true)if(!(f===false&&isEmpty(a))){if(isNaN(a))return[false,g+": "+djQuery.jgrid.edit.msg.integer,""];if(a%1!==0||a.indexOf(".")!=-1)return[false,g+": "+djQuery.jgrid.edit.msg.integer,""]}if(e.date===true)if(!(f===false&&isEmpty(a))){b=c.p.colModel[b].formatoptions&&c.p.colModel[b].formatoptions.newformat?c.p.colModel[b].formatoptions.newformat:c.p.colModel[b].datefmt||"Y-m-d";if(!checkDate(b,a))return[false,g+": "+djQuery.jgrid.edit.msg.date+
" - "+b,""]}if(e.time===true)if(!(f===false&&isEmpty(a)))if(!checkTime(a))return[false,g+": "+djQuery.jgrid.edit.msg.date+" - hh:mm (am/pm)",""];if(e.url===true)if(!(f===false&&isEmpty(a))){d=/^(((https?)|(ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;if(!d.test(a))return[false,g+": "+djQuery.jgrid.edit.msg.url,""]}if(e.custom===true)if(!(f===false&&isEmpty(a)))if(djQuery.isFunction(e.custom_func)){a=e.custom_func.call(c,a,g);return djQuery.isArray(a)?
a:[false,djQuery.jgrid.edit.msg.customarray,""]}else return[false,djQuery.jgrid.edit.msg.customfcheck,""]}return[true,"",""]};
...
q){q=m.p.postData[q.sFilter];if(typeof q=="string")q=a.jgrid.parse(q);if(q){q.groupOp&&m.SearchFilter.setGroupOp(q.groupOp);if(q.rules){var A,F=0,K=q.rules.length;for(A=false;F<K;F++){A=q.rules[F];if(A.field!==undefined&&A.op!==undefined&&A.data!==undefined)(A=m.SearchFilter.setFilter({sfref:m.SearchFilter.d$.find(".sf:last"),filter:a.extend({},A)}))&&m.SearchFilter.add()}}}}function o(m){if(f.onClose){var q=f.onClose(m);if(typeof q=="boolean"&&!q)return}m.hide();f.overlay===true&&a(".jqgrid-overlay:first",
...
operators:s,onClose:o,resetText:f.Reset,searchText:f.Find,windowTitle:f.caption,rulesText:f.rulesText,matchText:f.matchText,onSearch:v,onReset:B,stringResult:f.stringResult,ajaxSelectOptions:a.extend({},a.jgrid.ajaxOptions,w.p.ajaxSelectOptions||{}),clone:f.cloneSearchRowOnAdd});a(".ui-widget-overlay","#"+i).remove();w.p.direction=="rtl"&&a(".ui-closer","#"+i).css("float","left");if(f.drag===true){a("#"+i+" table thead tr:first td:first").css("cursor","move");if(djQuery.fn.jqDrag)a("#"+i).jqDrag(a("#"+
...
a.each(g.p.colModel,function(){if(this.name==r&&this.editoptions&&a.isFunction(this.editoptions.custom_value)){try{c[r]=this.editoptions.custom_value(a("#"+r,"#"+j),"get");if(c[r]===undefined)throw"e1";}catch(p){p=="e1"?info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,djQuery.jgrid.edit.bClose):info_dialog(djQuery.jgrid.errors.errcap,p.message,djQuery.jgrid.edit.bClose)}return true}})}else{switch(a(this).get(0).type){case "checkbox":if(a(this).attr("checked"))c[this.name]=
...
p,"#"+t),"set",z);else throw"e1";}catch(T){T=="e1"?info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,djQuery.jgrid.edit.bClose):info_dialog(djQuery.jgrid.errors.errcap,T.message,djQuery.jgrid.edit.bClose)}break}I++}});I>0&&a("#id_g","#"+j).val(e)}}}function B(){var e=[true,"",""],r={},t=g.p.prmNames,p,I;if(a.isFunction(d.beforeCheckValues)){var z=d.beforeCheckValues(c,a("#"+h),c[g.p.id+"_id"]=="_empty"?t.addoper:t.editoper);if(z&&typeof z==="object")c=z}for(var x in c)if(c.hasOwnProperty(x)){e=
...
var x=e.find("tr.sf"),G=x.find("td.fields"),y=x.find("td.ops"),o=x.find("td.data"),s="";djQuery.each(f.operators,function(){s+=m(this.op,this.text)});s=w("default",s,true);y.append(s);o.append("<input type='text' class='default' style='display:none;' />");var t="",z=false,p=false;djQuery.each(l,function(a){t+=m(this.itemval,this.text);if(this.ops!=null){z=true;var b="";djQuery.each(this.ops,function(){b+=m(this.op,this.text)});b=w("field"+a,b,true);y.append(b)}if(this.dataUrl!=null){if(a>n)n=a;p=true;
var d=this.dataEvents,c=this.dataInit,g=this.buildSelect;djQuery.ajax(djQuery.extend({url:this.dataUrl,complete:function(h){h=g!=null?djQuery("<div />").append(g(h)):djQuery("<div />").append(h.responseText);h.find("select").addClass("field"+a).hide();o.append(h.html());c&&E(".field"+a,c);d&&F(".field"+a,d);a==n&&e.find("tr.sf td.fields select[name='field']").change()}},f.ajaxSelectOptions))}else if(this.dataValues!=null){p=true;var i="";djQuery.each(this.dataValues,function(){i+=m(this.value,this.text)});
i=w("field"+a,i,true);o.append(i)}else if(this.dataEvents!=null||this.dataInit!=null){p=true;i="<input type='text' class='field"+a+"' />";o.append(i)}this.dataInit!=null&&a!=n&&E(".field"+a,this.dataInit);this.dataEvents!=null&&a!=n&&F(".field"+a,this.dataEvents)});t="<select name='field'>"+t+"</select>";G.append(t);l=G.find("select[name='field']");z?l.change(function(a){var b=a.target.selectedIndex;a=djQuery(a.target).parents("tr.sf").find("td.ops");a.find("select").removeAttr("name").hide();b=a.find(".field"+
b);if(b[0]==null)b=a.find(".default");b.attr("name","op").show();return false}):y.find(".default").attr("name","op").show();p?l.change(function(a){var b=a.target.selectedIndex;a=djQuery(a.target).parents("tr.sf").find("td.data");a.find("select,input").removeClass("vdata").hide();b=a.find(".field"+b);if(b[0]==null)b=a.find(".default");b.show().addClass("vdata");return false}):o.find(".default").show().addClass("vdata");if(z||p)l.change();e.find(".ui-state-default").hover(C,C).mousedown(D).mouseup(D);
e.find(".ui-closer").click(function(){f.onClose(djQuery(e.selector));return false});e.find(".ui-del").click(function(a){a=djQuery(a.target).parents(".sf");if(a.siblings(".sf").length>0){f.datepickerFix===true&&djQuery.fn.datepicker!==undefined&&a.find(".hasDatepicker").datepicker("destroy");a.remove()}else{a.find("select[name='field']")[0].selectedIndex=0;a.find("select[name='op']")[0].selectedIndex=0;a.find(".data input").val("");a.find(".data select").each(function(){this.selectedIndex=0});a.find("select[name='field']").change(function(b){b.stopPropagation()})}return false});
e.find(".ui-add").click(function(a){a=djQuery(a.target).parents(".sf");var b=a.clone(true).insertAfter(a);b.find(".ui-state-default").removeClass("ui-state-hover ui-state-active");if(f.clone){b.find("select[name='field']")[0].selectedIndex=a.find("select[name='field']")[0].selectedIndex;if(b.find("select[name='op']")[0]!=null)b.find("select[name='op']").focus()[0].selectedIndex=a.find("select[name='op']")[0].selectedIndex;var d=b.find("select.vdata");if(d[0]!=null)d[0].selectedIndex=a.find("select.vdata")[0].selectedIndex}else{b.find(".data input").val("");
b.find("select[name='field']").focus()}f.datepickerFix===true&&djQuery.fn.datepicker!==undefined&&a.find(".hasDatepicker").each(function(){var c=djQuery.data(this,"datepicker").settings;b.find("#"+this.id).unbind().removeAttr("id").removeClass("hasDatepicker").datepicker(c)});b.find("select[name='field']").change(function(c){c.stopPropagation()});return false});e.find(".ui-search").click(function(){var a=djQuery(e.selector),b,d=a.find("select[name='groupOp'] :selected").val();b=f.stringResult?'{"groupOp":"'+
d+'","rules":[':{groupOp:d,rules:[]};a.find(".sf").each(function(c){var g=djQuery(this).find("select[name='field'] :selected").val(),i=djQuery(this).find("select[name='op'] :selected").val(),h=djQuery(this).find("input.vdata,select.vdata :selected").val();h+="";h=h.replace(/\\/g,"\\\\").replace(/\"/g,'\\"');if(f.stringResult){if(c>0)b+=",";b+='{"field":"'+g+'",';b+='"op":"'+i+'",';b+='"data":"'+h+'"}'}else b.rules.push({field:g,op:i,data:h})});if(f.stringResult)b+="]}";f.onSearch(b);return false});e.find(".ui-reset").click(function(a,
b){a=djQuery(e.selector);a.find(".ui-del").click();a.find("select[name='groupOp']")[0].selectedIndex=0;f.onReset(b);return false});e.find(".ui-add-last").click(function(){var a=djQuery(e.selector+" .sf:last"),b=a.clone(true).insertAfter(a);b.find(".ui-state-default").removeClass("ui-state-hover ui-state-active");b.find(".data input").val("");b.find("select[name='field']").focus();f.datepickerFix===true&&djQuery.fn.datepicker!==undefined&&a.find(".hasDatepicker").each(function(){var d=djQuery.data(this,
"datepicker").settings;b.find("#"+this.id).unbind().removeAttr("id").removeClass("hasDatepicker").datepicker(d)});b.find("select[name='field']").change(function(d){d.stopPropagation()});return false});this.setGroupOp=function(a){selDOMobj=e.find("select[name='groupOp']")[0];var b={},d=selDOMobj.options.length,c;for(c=0;c<d;c++)b[selDOMobj.options[c].value]=c;selDOMobj.selectedIndex=b[a];djQuery(selDOMobj).change(function(g){g.stopPropagation()})};this.setFilter=function(a){var b=a.sfref;a=a.filter;
...
k,H)};djQuery.fn.searchFilter.version="1.2.9";
djQuery.fn.searchFilter.defaults={clone:true,datepickerFix:true,onReset:function(k){alert("Reset Clicked. Data Returned: "+k)},onSearch:function(k){alert("Search Clicked. Data Returned: "+k)},onClose:function(k){k.hide()},groupOps:[{op:"AND",text:"all"},{op:"OR",text:"any"}],operators:[{op:"eq",text:"is equal to"},{op:"ne",text:"is not equal to"},{op:"lt",text:"is less than"},{op:"le",text:"is less or equal to"},{op:"gt",text:"is greater than"},{op:"ge",text:"is greater or equal to"},{op:"in",text:"is in"},
...
}catch(q){q=="e1"&&info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_value' "+b.jgrid.edit.msg.nodefined,djQuery.jgrid.edit.bClose);q=="e2"?info_dialog(djQuery.jgrid.errors.errcap,"function 'custom_value' "+b.jgrid.edit.msg.novalue,djQuery.jgrid.edit.bClose):info_dialog(djQuery.jgrid.errors.errcap,q.message,djQuery.jgrid.edit.bClose)}break}if(g!=a.p.savedRow[c].v){if(b.isFunction(a.p.beforeSaveCell))if(c=a.p.beforeSaveCell.call(a,a.rows[d].id,j,f,d,e))g=f=c;var r=checkValues(f,e,a);if(r[0]===true){c=
...
f.s)[0])&&s(f);return!a},o=function(a,f,c){return a.each(function(){var d=this._jqm;b(f).each(function(){if(!this[c]){this[c]=[];b(this).click(function(){for(var h in{jqmShow:1,jqmHide:1})for(var g in this[h])i[this[h][g]]&&i[this[h][g]].w[h](this);return e})}this[c].push(d)})})}})(djQuery);
...
k:j.k}:false;try{b("input.hasDatepicker",d[0]).datepicker("hide")}catch(p){}b(document).mousemove(b.jqDnR.drag).mouseup(b.jqDnR.stop);return false})})},k=function(a){return parseInt(d.css(a))||false};f1=function(a){return parseInt(f.css(a))||false}})(djQuery);
...
else(a=b("td.sgexpanded",c)[0])&&b(a).trigger("click")}})}})})(djQuery);
...
0);return Math.max(parseFloat(b),parseFloat(a[c]||0))},count:function(b,c,a){if(b==="")b=0;return a.hasOwnProperty(c)?b+1:0},avg:function(b,c,a){return parseFloat(b||0)+parseFloat(a[c]||0)}}})})(djQuery);
...
b,c)});return a},delTreeNode:function(b){return this.each(function(){var c=this;if(c.grid&&c.p.treeGrid){var a=d(c).jqGrid("getInd",b,true);if(a){var f=d(c).jqGrid("getNodeChildren",a);if(f.length>0)for(var e=0;e<f.length;e++)d(c).jqGrid("delRowData",f[e].id);d(c).jqGrid("delRowData",a.id)}}})}})})(djQuery);
...
"}]}")}break}}});return d},excelExport:function(a){a=b.extend({exptype:"remote",url:null,oper:"oper",tag:"excel",exportOptions:{}},a||{});return this.each(function(){if(this.grid){var d;if(a.exptype=="remote"){d=b.extend({},this.p.postData);d[a.oper]=a.tag;d=djQuery.param(d);d=a.url.indexOf("?")!=-1?a.url+"&"+d:a.url+"?"+d;window.location=d}}})}})})(djQuery);
...
j&&a.beforeShowForm(b("#"+d));viewModal("#"+f.themodal,{gbox:"#gbox_"+e,jqm:a.jqModal,jqM:true,modal:a.modal})}k&&a.afterShowForm(b("#"+d))}})}})})(djQuery);
...
b},getPostDataItem:function(a){var b=this[0];if(b.grid)return b.p.postData[a]},removePostDataItem:function(a){var b=this[0];b.grid&&delete b.p.postData[a]},getUserData:function(){var a=this[0];if(a.grid)return a.p.userData},getUserDataItem:function(a){var b=this[0];if(b.grid)return b.p.userData[a]}})})(djQuery);
function tableToGrid(n,o){djQuery(n).each(function(){if(!this.grid){djQuery(this).width("99%");var a=djQuery(this).width(),d=djQuery("input[type=checkbox]:first",djQuery(this)),b=djQuery("input[type=radio]:first",djQuery(this));d=d.length>0;b=!d&&b.length>0;var l=d||b,c=[],g=[];djQuery("th",djQuery(this)).each(function(){if(c.length===0&&l){c.push({name:"__selection__",index:"__selection__",width:0,hidden:true});g.push("__selection__")}else{c.push({name:djQuery(this).attr("id")||djQuery.trim(djQuery.jgrid.stripHtml(djQuery(this).html())).split(" ").join("_"),
index:djQuery(this).attr("id")||djQuery.trim(djQuery.jgrid.stripHtml(djQuery(this).html())).split(" ").join("_"),width:djQuery(this).width()||150});g.push(djQuery(this).html())}});var f=[],h=[],i=[];djQuery("tbody > tr",djQuery(this)).each(function(){var j={},e=0;djQuery("td",djQuery(this)).each(function(){if(e===0&&l){var k=djQuery("input",djQuery(this)),m=k.attr("value");h.push(m||f.length);k.attr("checked")&&i.push(m);j[c[e].name]=k.attr("value")}else j[c[e].name]=djQuery(this).html();e++});e>0&&f.push(j)});
djQuery(this).empty();djQuery(this).addClass("scroll");djQuery(this).jqGrid(djQuery.extend({datatype:"local",width:a,colNames:g,colModel:c,multiselect:d},o||{}));for(a=0;a<f.length;a++){b=null;if(h.length>0)if((b=h[a])&&b.replace)b=encodeURIComponent(b).replace(/[.\-%]/g,"_");if(b===null)b=a+1;djQuery(this).jqGrid("addRowData",b,f[a])}for(a=0;a<i.length;a++)djQuery(this).jqGrid("setSelection",i[a])}})};
...
"#gview_"+j.p.id);delete b._alsoResize_;a("#gbox_"+j.p.id).resizable(b)}})}})})(djQuery);
