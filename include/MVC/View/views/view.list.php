...
/* BEGIN ModuleSurfer ::TAG 0121r7b2:: */
        if(isset($_REQUEST['disable_ms'])) $GLOBALS['EMDisable_ms'] = $_REQUEST['disable_ms'];
        if(!isset($_REQUEST['msPreventQuery']))
/* END ModuleSurfer ::TAG 0121r7b2:: */ 
...
$this->storeQuery->populateRequest();
    ...
    function listViewProcess(){
        ...
        $this->lv->searchColumns = $this->searchForm->searchColumns;
         ...
         /* BEGIN ModuleSurfer ::TAG 0121r75h:: */
        if (!file_exists('modules/'.$this->module.'/Calendar.php') && file_exists('custom/include/ModuleSurfer/ListView/SurferListView.php')) {
            require_once('custom/include/ModuleSurfer/ListView/SurferListView.php');
            if (class_exists('SurferListView')) {
                $slv = new SurferListView($this);
                if ($slv->configSync()) {
                    $tpl = 'include/ListView/ListViewGeneric.tpl';
                }
                else {
                    $tpl = 'custom/include/ModuleSurfer/ListView/tpl/ListViewGeneric.tpl';
...
/* END ModuleSurfer ::TAG 0121r75h:: */
if(!$this->headers)
            return;
        if(empty($_REQUEST['search_form_only']) || $_REQUEST['search_form_only'] == false){
            $this->lv->ss->assign("SEARCH",true);
 /* BEGIN ModuleSurfer ::TAG 0121r75i:: */
            $this->lv->setup($this->seed, $tpl, $this->where, $this->params);
            if ($slv) $slv->additionalTplParams();
/* END ModuleSurfer ::TAG 0121r75i:: */
