<div style="float: left; margin: 0px 0px 10px 0px; padding: 0px; width:100%; border-top: 2px solid #4E8CCF;">
  <div style="float: left; width: 33%; padding: 2px">
    <span style="font-weight: bold;">Unique Id:</span> {$bean->id}
  </div><div style="float: left; width: 22%; padding: 2px">
      <input type="button" value="Request payment" onclick="$('#payment_modal').toggle();" />
      <div id="payment_modal" style="background-color: #FFFFFF;   border: 1px solid #87CEEB;   border-radius: 5px;   display: none;   float: left; padding: 20px; position: absolute; top: 250px; width: 210px;">
          <h3>Email for payment request</h3>
          <div id="email_err"></div>
          <br />
          <label>Amount ($):</label> 
          <br />
          <input type="text" id="request_amount" value="{$request_amount}" name="amount" />
          <br />
          <br />
          <label>Email:</label>
          <br />
          <input type="text" id="client_email_req" value="{$bean->email1}" name="email" />
          <br /><br />
          <input type="button" value="Send" onclick="send_payment_request_email();" />        
      </div>
  </div><div style="float: left; width: 22%; padding: 2px">
    <form target="_blank" method="POST" action="/sugarcrm/index.php?module=abc_Creditors&action=index">
      <input type="hidden" value="basic_search" name="searchFormTab" />
      <input type="hidden" value="{$bean->id}" name="abc_creditors_contacts_id_basic" />
      <input type="hidden" value="{$bean->name}" name="abc_creditors_contacts_name_basic" />
      <input type="hidden" value="true" name="query" />
      <input type="hidden" value="" name="orderBy" />
      <input type="hidden" value="" name="sortOrder" />
      <input type="hidden" value="" name="name_basic" />
      <input type="hidden" value="0" name="current_user_only_basic" />
      <input type="hidden" value="Search" name="button" />
      <input type="submit" value="Show creditors" />
    </form>
  </div>
  <div style="float: left; width: 20%; padding: 2px"><form action="">
  <input type="hidden" value="{$bean->id}" name="abc_creditors_contacts_id_basic" id="c_id"/>
 <input type="button" value="Send Email" name="emailsend" id="sendemail"/>
 </form>
</div>
 <div style="float: left; width: 20%; padding: 2px"><form action='' style="width: 8%; left: 50%; position: relative;">
            <input type='hidden' name='email' id='email12' value='' >
            <input type='button' value='Show Templates' name='send' id='sent_detail' >
        </form></div>
</div>
