	...
	**/
	...
	private function save_to_sugar($data){
		...
		if($rel_table = $this->get_rel_table()){
            if(nocrm and strtolower($this->focus_type) == 'contacts'){
                $sql = "INSERT INTO `sc_stripepayments_contacts_c` (`id`, `date_modified`, `deleted`, `sc_stripepayments_contactscontacts_ida`, `sc_stripepayments_contactssc_stripepayments_idb`) VALUES (UUID(), NOW(), '0', '$this->focus_id', '$this->payment_record_id');";
		...
		if(strtolower($this->focus_type) == 'contacts'){
		    
		    $sql = "SELECT * FROM `contacts_cstm` where id_c = '$this->focus_id';";
            $result = $GLOBALS['db']->query($sql);

            while($row = $GLOBALS['db']->fetchByAssoc($result) )
            {
                //Use $row['id'] to grab the id fields value
                $quoted_amount = $row['quoted_amount_c'];
                $paid_amount = $row['paid_amount_c'];
            }
            
            //print_r($row);
            //echo "aa $paid_amount  bb $quoted_amount";
            $focus->paid_amount_c = $paid_amount + $sp->amount;
		    $balance_amount = $quoted_amount - $focus->paid_amount_c;
		}
		$focus_cstm_tablename = strtolower($this->focus_type).'_cstm';

        

        if(nocrm){
		    $sql="update contacts_cstm set stripe_customer_id_c = '".$focus->stripe_customer_id_c."' , paid_amount_c = '". $focus->paid_amount_c ."' , balance_amount_c = '". $balance_amount ."' where id_c = '$this->focus_id';";
		    //echo $sql;
		    $result = $GLOBALS['db']->query($sql);        
        } else{
            $focus->save();        
        ...
        // More headers
        $headers .= 'From: <credirguru@example.com>' . "\r\n";

        $subject = "Payment recieved by credit guru";
        
        $amount_cents = $_POST['amount'] * 100;
        $message = "Hi $focus->first_name $focus->last_name, <br /> A payment of $ $sp->amount has been recieved, paid by $sp->cc_type card **$sp->cc_last_four. <br /><br />Thanks,<br />Credit guru";
        // message lines should not exceed 70 characters (PHP rule), so wrap it
        $message = wordwrap($message, 70);
        // send mail
        if(mail("$focus->email1",$subject,$message,$headers)){
            //echo "Email sent";
        } else{
        ...
        $User->retrieve($focus->assigned_user_id);	//retrieve user data
        $email_add = $User->email1;	//get user email address

        $User2 = new User();		//user object
        $User2->retrieve(1);	//retrieve user data
        $email_add2 = $User2->email1;	//get user email address
//echo "$email_add , $email_add2";
        $subject = "Payment recieved from $focus->first_name $focus->last_name";
        $message = "Hi, <br /> A payment of $ $sp->amount has been recieved via $sp->cc_type card **$sp->cc_last_four from $focus->first_name $focus->last_name.";

        if(mail("$email_add , $email_add2",$subject,$message,$headers)){
            //echo "Email sent";
        } else{
            //echo "Failed to send email";
        }
