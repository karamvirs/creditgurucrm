...
{php} 
$this->_tpl_vars['disableMS'] = $_REQUEST['action'] == 'UnifiedSearch' || in_array($_REQUEST['module'], array('Activities', 'Calendar', 'Employees', 'Project', 'Reports')) || file_exists('modules/'.$_REQUEST['module'].'/Calendar.php'); 
$this->_tpl_vars['disableMSLabel'] = $GLOBALS['app_strings']['LBL_MODULESURFER']['LBL_TPL']['LBL_DISABLE_MODULESURFER']; 
{/php}
{if !$disableMS}
<div style="padding-top:10px;">
{if (!$moduleSurferDisableManual || !$moduleSurferDisableManual.disableManual) && !$moduleSurferForceDisabled}
    <input type="checkbox" name="surfer-disable" onclick="javascript:document.location.href = 'index.php?module={$pageData.bean.moduleDir}&action=index&disable_ms=0';" checked="checked"/><span style="position:relative; top:0px; left:3px;">{$disableMSLabel}</span>
{/if}
</div>
{php}
if (!class_exists('SurferListView') && !@$_SESSION['msKeyExpiredMessageSent']) {
	echo '<script language="javascript">alert("'.$GLOBALS['app_strings']['LBL_MODULESURFER']['LBL_TPL']['LBL_DRC_OUT_OF_DATE_ERROR'].'");</script>';
	$_SESSION['msKeyExpiredMessageSent'] = true;
}
{/php}
{/if}{* END ModuleSurfer ::TAG 0121r75f:: *}
{/if}
{if $contextMenus}
